...
import { X, Swords, Save, Loader2, Image as ImageIcon, Upload, Info } from 'lucide-react';
...
export default function CharacterModal({ isOpen, onClose, onSuccess, character }: CharacterModalProps) {
    ...
    try {
      ...
      if (pendingFile) {
        try {
          ...
          await characterService.uploadCharacterImage(charResult.id, pendingFile);
        } catch (uploadErr) {
              ...
              <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.02]">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#c9a84c]/10 flex items-center justify-center border border-[#c9a84c]/20">
                    {isEdit ? <Info className="w-4 h-4 text-[#c9a84c]" /> : <Swords className="w-4 h-4 text-[#c9a84c]" />}
                  </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                ...
                <div className="flex flex-col items-center gap-6 py-8 bg-white/5 rounded-2xl border border-dashed border-white/10 group hover:border-[#c9a84c]/30 transition-all">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-2.5 bg-[#c9a84c]/10 border border-[#c9a84c]/20 text-[#c9a84c] rounded-xl font-cinzel text-[10px] font-bold tracking-widest hover:bg-[#c9a84c]/20 transition-all shadow-lg"
                  >
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-cinzel text-[#c9a84c]/60 uppercase tracking-widest font-bold ml-1">Nome da Lenda</label>
                    <input 
                      type="text" 
                      value={name} 
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-cinzel text-[#c9a84c]/60 uppercase tracking-widest font-bold ml-1">Sistema de Reinos</label>
                    <select 
                      value={system} 
                    ...
                    >
                      <option value="DND_5E" className="bg-[#0c0a08] text-[#e8d5b0]">Dungeons & Dragons 5e</option>
                      <option value="PATHFINDER" className="bg-[#0c0a08] text-[#e8d5b0]">Pathfinder 2e</option>
                      <option value="OTHER" className="bg-[#0c0a08] text-[#e8d5b0]">Outro Sistema</option>
                    </select>
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[10px] font-cinzel text-[#c9a84c]/60 uppercase tracking-widest font-bold ml-1">Crônicas do Passado (Backstory)</label>
                    <textarea 
                      value={backstory} 
                      onChange={e => setBackstory(e.target.value)} 
                      placeholder="Conte como esta lenda começou..."
                      rows={4}
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-[#e8d5b0] focus:border-[#c9a84c]/40 focus:outline-none transition-all placeholder:text-white/10 resize-none min-h-[120px] custom-scrollbar"
                    />
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button 
                    type="submit" 
                    disabled={loading}
                    className="flex-[2] flex items-center justify-center gap-2 px-6 py-3 bg-[#c9a84c] text-[#0c0a08] rounded-xl font-cinzel text-xs font-bold tracking-widest hover:shadow-[0_0_20px_rgba(201,168,76,0.3)] transition-all disabled:opacity-50"
                  >
                    {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                    {isEdit ? 'SALVAR CRÔNICA' : 'INICIAR JORNADA'}
                  </button>
                </div>
              </form>
            </motion.div>
